...
def prepare_data(data, year=2017):
    ...
    3) convert the `value` column to numeric
    ...
    data_train.reset_index(inplace=True)
    data_test.reset_index(inplace=True)
    data_train['ds'] = pd.to_datetime(data_train['datetime'])
    data_test['ds'] = pd.to_datetime(data_test['datetime'])
    data_train['value'] = pd.to_numeric(data_train['value'])
    data_test['value'] = pd.to_numeric(data_test['value'])
    data_train = data_train[['ds', 'value']]
    data_test = data_test[['ds', 'value']]
    return data_train, data_test
'''
